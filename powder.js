 var Game = function(canvas,tip_area,point_area,playername) {
    ...
    this.config = {
        ...
        object_deploy: [
            ...
            ["water","_water","good",1.1,1.2,19,19],
...
Game.prototype.update_object = function() {
    ...
    if(rand > this.config.object_speed * 1000) {
        this.object.push(this.getObject());
    }
...
Game.prototype.update_game = function() {
    ...
    // change deploy
    if(Math.log10(this.player.life) > 25) {
        this.config.object_deploy[6][3] = 0.1 ** (Math.log10(this.player.life) / 2)
    } else {
        this.config.object_deploy[6][3] = this.config.object_deploy[6][4];
    }
    // achievement
...
Game.prototype.draw = function() {
    ...
    if(this.config.dead) {
        ...
        this.ctx.font = "30px 宋体"
        this.ctx.fillText("You lost 运动力！", this.width / 2 - 100,this.height / 2 - 30);    
    }
};
